...
- `src/backend/` — Node.js/Express server (TypeScript, ESM)
- `src/frontend/` — React SPA (TypeScript, Vite, Tailwind CSS)
- `scripts/` — Build and deployment scripts (`build.sh`, `start.sh`)
- `app/` — Build output directory (generated by `scripts/build.sh`)
...
- **GlobalDownloadNotifier** — Floating notification for download status changes
- **ToastContainer** — Container for toast messages
- **icons** — shared SVG icon components used by Sidebar, MobileNav, and MobileHeader:
  - `HomeIcon`, `AccountsIcon`, `SearchIcon`, `DownloadsIcon`, `SettingsIcon` (navigation)
  - `SunIcon`, `MoonIcon`, `SystemIcon` (theme toggle)
...
`accountHash` is a SHA-256 hash of the account identifier (DSID, Apple ID, or email). It is treated as **public, non-secret data** — it identifies which account owns a download but does not grant any privileged access. No authentication is bound to it. This is by design: the server is a blind proxy and does not manage user sessions.

**Salt Support**: The frontend supports optional salting via `VITE_ACCOUNT_HASH_SALT` environment variable. When set, the hash is computed as `SHA-256(identifier + ":" + salt)` to prevent rainbow table attacks and cross-instance enumeration. If not set, it falls back to plain hashing for backward compatibility.
...
**Style Constants for Repeated Patterns**: For complex className strings that are reused within a single component (e.g., form inputs), extract them as constants at the top of the file. This ensures consistency and makes future updates easier. Example:
...
Note: `transition-colors` is allowed on interactive elements (inputs, buttons) but must never be used on static containers.
